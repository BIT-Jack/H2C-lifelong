...
cl_methods = ['vanilla', 'vanilla', 'vanilla', 'vanilla', 'h2c', 'h2c', 'h2c', 'h2c']
bf = 2000
buffers = [0, 0, 0, 0, bf, bf, bf, bf]
learned_tasks = [2, 3, 4, 5, 2, 3, 4, 5]  # Assuming all are the same, you can modify each entry if needed
# learned_tasks=[3, 4, 5, 6, 3, 4, 5, 6]
test_sce = 'FT'
# mapfile = './mapfiles/DR_USA_Intersection_MA.osm'
mapfile = './mapfiles/DR_USA_Roundabout_FT.osm'
# mapfile = './mapfiles/DR_CHN_Roundabout_LN.osm'
# mapfile = './mapfiles/DR_CHN_Merging_ZS2.osm'
# mapfile = './mapfiles/DR_DEU_Roundabout_OF.osm'
# mapfile = './mapfiles/DR_USA_Intersection_EP0.osm'
...
data_error = np.load('./logging/results_record/fde_mr_'+cl_methods[0]+'_buffer'+str(buffers[0])+'_'+str(learned_tasks[2])+'tasks_learned_test_on_'+test_sce+'.npz')
...
# case_id = np.argmax(case_fde) #OF194
...
fig = plt.figure(figsize=(7.15, 3.2))
gs = gridspec.GridSpec(2, 4, figure=fig, left=0.044, right=0.99, top=0.889, bottom=0.068, wspace=0.057, hspace=0.208)
...
for i in range(8):
    ...
    for ii in range(1, 26):
        ...
        if sv_length == 0 and sv_width == 0:
            ...
            h_sv, = ax.plot([], [], color='b', label='observed trajectories of SVs')  # Dummy for legend
    ...
    if i in [0,1,2,3]:
        if i==0:
            ax.set_title('Vanilla: after '+str(learned_task)+' tasks', fontsize=8, pad=2)
        else:
            ax.set_title('Vanilla: after '+str(learned_task)+' tasks', fontsize=8, pad=2)
    else:
        if i==4:
            ax.set_title('H2C (ours): after '+str(learned_task)+' tasks', fontsize=8, pad=2)
        else:
            ax.set_title('H2C (ours): after '+str(learned_task)+' tasks', fontsize=8, pad=2)
...
plt.tight_layout()
plt.savefig('./traj_vis_bf2000_FT_case491.svg')
# plt.show()
